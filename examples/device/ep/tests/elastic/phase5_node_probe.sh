...
python3 - "${HOST}" "${STAGE}" > "${RUN_DIR}/${STAGE}_${HOST}_nvlink.csv" 2>/dev/null <<'PY'
import csv, sys
sys.path.insert(0, "/workspace/lishapira/nixl/examples/device/ep/tests/elastic")
import fault_artifacts as fa

host = sys.argv[1]
raw = fa._run_nvsmi_errorcounters()
parsed = fa._parse_nvsmi_errorcounters(raw) if raw else {}
writer = csv.writer(sys.stdout)
writer.writerow(["host", "gpu", "link"] + list(fa._NVLINK_FIELDS))
for gpu, links in parsed.items():
    for link, values in links.items():
        writer.writerow([host, gpu, link] + [values.get(f, "") for f in fa._NVLINK_FIELDS])
PY
